...
@icms_router.get("/states", response=list[StateSchema])
...
@icms_router.get("/ncm-groups", response=list[NCMGroupSchema])
...
@icms_router.get("/ncm", response=list[NCMSchema])
...
@icms_router.get("/rates", response=list[ICMSRateSchema])
def list_icms_rates(request):
    return service.list_icms_rates()


@icms_router.get("/rates/{rate_id}", response=ICMSRateSchema)
def get_icms_rate(request, rate_id: uuid.UUID):
    return service.get_icms_rate(rate_id)


@icms_router.patch("/rates/{rate_id}", response=ICMSRateSchema)
def update_icms_rate(request, rate_id: uuid.UUID, payload: ICMSRateUpdateSchema):
    return service.update_icms_rate(rate_id, payload)
